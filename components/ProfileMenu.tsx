...
export function ProfileMenu({
  ...
  const lastOpenTokenRef = useRef<number | undefined>(openToken);
  ...
  useEffect(() => {
    if (typeof openToken !== "number") return;
    if (lastOpenTokenRef.current === undefined) {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        mobileCentered ? (
          <div
            className="fixed inset-0 z-[170] flex items-center justify-center bg-black/55 p-4"
            onClick={() => setOpen(false)}
          >
            <div
              className="z-[180] max-h-[min(80vh,calc(100dvh-5rem))] w-[min(calc(100vw-1.5rem),20rem)] origin-center overflow-y-auto overscroll-contain rounded-2xl border border-white/10 bg-surface-container-high/95 p-4 shadow-2xl backdrop-blur-xl m2m-rise"
              role="menu"
